        ...
        <div className='flex flex-col mt-[20px] gap-1.5 border-t border-[#80808059] border-dashed pt-[20px]'>
            {
                sideMenusData?.map((data) => {
                    return (
                        <a href={data?.url} key={data?.id} className={cn('flex gap-2 items-center  hover:bg-[#b0b0eb59]  rounded-[6px] py-[8px] px-[14px]',path == data?.url && active)}>
                            <Image src={data?.icon} alt='arrow icon' className={cn('w-[25px] h-[25px] object-contain',path == data?.url ? 'active-filter':  filterBlack)}></Image>
